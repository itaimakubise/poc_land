...
with tab2:
    ...
    # --- UPDATED CLEAN STREET RISK INDEX ---
    st.markdown("---")
...
with tab3:
    ...
    with c1:
        ...
        fig_speed = px.bar(speed_data, x='crash_speed_limit', y='count', color='count', color_continuous_scale='YlOrRd', title="Volume by Speed Limit")
        st.plotly_chart(fig_speed, use_container_width=True)
    with c2:
        drain_data = df.groupby('rpt_street_name')['Estimated Total Comprehensive Cost'].sum().nlargest(10).reset_index()
        fig_drain = px.bar(drain_data, x='Estimated Total Comprehensive Cost', y='rpt_street_name', orientation='h', color='Estimated Total Comprehensive Cost', color_continuous_scale='Purples', title="Top 10 Financial Drain Streets")
...
with tab4:
    ...
    fig_vru_pie = px.pie(values=vru_sums.values, names=['Pedestrian', 'Bicycle', 'Motorcycle', 'Motor Vehicle'], 
                         hole=0.5, title="Fatality Distribution", color_discrete_sequence=px.colors.sequential.Reds)
    st.plotly_chart(fig_vru_pie, use_container_width=True)

# --- RAW DATA VIEW ---
with st.expander("🔍 Detailed Records"):
    # Clean up column names in the raw view as well
    raw_display = df[['Crash timestamp', 'rpt_street_name', 'crash_speed_limit', 'Severity_Label', 'Estimated Total Comprehensive Cost']].copy()
    raw_display.columns = ['Timestamp', 'Street Name', 'Speed Limit', 'Severity', 'Comprehensive Cost']
    st.write(raw_display.sort_values(by='Comprehensive Cost', ascending=False).style.format({'Comprehensive Cost': '${:,.0f}'}))