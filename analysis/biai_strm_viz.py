...
with tab2:
    ...
    with map_col1:
        st.write("🔍 **Map Insights**")
        view_mode = st.radio("Switch Pattern View:", ["Cluster Heatmap", "Incident Detail Markers"])
        st.caption("Heatmap highlights 'Hot Zones' by Economic Drain. Markers show Speed vs Severity.")
        
    with map_col2:
        if view_mode == "Cluster Heatmap":
            # DENSITY HEATMAP: Weighted by Economic Impact
            fig_map = px.density_mapbox(df, lat='latitude', lon='longitude', z='Estimated Total Comprehensive Cost',
                                        ...
                                        mapbox_style="carto-darkmatter", title="Concentration of Economic Loss")
        else:
            # SCATTER MAPBOX: Sized by Speed, Colored by Severity
            fig_map = px.scatter_mapbox(df, lat='latitude', lon='longitude', color='Severity_Label', 
                                        size='crash_speed_limit', size_max=15,
                                        center=dict(lat=30.2672, lon=-97.7431), zoom=10,
                                        mapbox_style="carto-positron", 
                                        hover_data={'rpt_street_name':True, 'crash_speed_limit':True, 'Estimated Total Comprehensive Cost':True},
                                        title="Incident Specifics: Speed & Severity")
        
        fig_map.update_layout(margin={"r":0,"t":40,"l":0,"b":0}, height=600)
        st.plotly_chart(fig_map, use_container_width=True)
    
    st.info("💡 **Stakeholder Tip:** Use the 'Cluster Heatmap' to identify infrastructure funding priorities based on economic drain, and 'Incident Markers' for police enforcement planning in high-speed corridors.")
...
with tab4:
    ...
    fig_vru_pie = px.pie(values=vru_sums.values, names=['Pedestrian', 'Bicycle', 'Motorcycle', 'Motor Vehicle'], hole=0.5)
